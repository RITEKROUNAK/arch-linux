...
apptitle="Auto Arch Linux Installation Script By : Ritek"
baseurl=https://github.com/RITEKROUNAK/arch-linux/main
cpl=0
skipfont="0"
fspkgs=""
...
installbase()
{
	...
	if [[ ${cpl} == 1 ]]; then
		sed -i '/^\s*$/d' /tmp/autoarch-custom-package-list
		sed -i '/^#/ d' /tmp/autoarch-custom-package-list
		options=()
		while read pkg; do
			options+=("${pkg}" "" on)
		done < /tmp/autoarch-custom-package-list
...
while (( "$#" )); do
	case ${1} in
		--help)
			...
			echo "-t | --test ghusername ghbranch   Test autoarch script and language"
			exit 0
		;;
		-cpl | --custom-package-list)
			curl -L ${2} > /tmp/autoarch-custom-package-list
			if [[ "$?" != "0" ]]; then
		...
		-t | --test)
			curl -L https://raw.githubusercontent.com/${2}/autoarch/${3}/autoarch > autoarch
			sh autoarch -tt ${2} ${3}
			exit 0
		;;
		-tt) baseurl="https://raw.githubusercontent.com/${2}/autoarch/${3}";;
		-efi0) efimode=0;;
		-efi1)
			eficomputer=1
			efimode=1
